#include QMK_KEYBOARD_H
#include "features/mouse_turbo_click.h"



enum custom_keycodes
{
    //Autoclicker script mapped to key called "Turbo"
    TURBO = SAFE_RANGE,
};

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

    [0] = LAYOUT_pad_4x3(
        //My main macro layer
        KC_TRNS,   KC_TRNS,   KC_TRNS,   KC_MPLY,
        TURBO ,   KC_6,   KC_7,   KC_8,
        LCTL(KC_C),   LCTL(KC_V),   KC_F13,   TG(1)
      
    ),
    [1] = LAYOUT_pad_4x3(
        // Prob gonna use for AHK
        KC_6,   KC_2,   KC_3,   KC_4,
        KC_5,   KC_6,   KC_7,   KC_8,
        KC_9,   KC_A,   MO(3),   TG(2)
      
    ), 
    [2] = LAYOUT_pad_4x3(
        // Utility Layer
        KC_9,   KC_2,   KC_3,   KC_4,
        KC_5,   KC_6,   KC_7,   KC_8,
        KC_9,   KC_A,   KC_B,   TG(4)
      
    ),
    [3] = LAYOUT_pad_4x3(
        //Momentary Macro Layer
        QK_BOOT,   KC_2,   KC_3,   KC_4,
        KC_5,   KC_6,   KC_7,   KC_8,
        KC_9,   KC_A,   KC_B,   KC_C
      
    ),
    [4] = LAYOUT_pad_4x3(
        //Numberpad Layer
        TG(5),   KC_1,   KC_4,   KC_7,
        KC_0,   KC_2,   KC_5,   KC_8,
        KC_ENT,   KC_3,   KC_6,   KC_9
      
    ),
    [5] = LAYOUT_pad_4x3(
        //Fusion 360 Layer
        KC_TRNS,   KC_TRNS,   KC_TRNS,   KC_TRNS,
        KC_TRNS,   KC_TRNS,   KC_TRNS,   KC_TRNS,
        KC_TRNS,   KC_TRNS,   KC_TRNS,   TO(0)
      
    ),                          
};

//VIAL config for Encoders
#if defined(ENCODER_MAP_ENABLE)
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {
    [0] =   { ENCODER_CCW_CW(KC_MS_WH_UP, KC_MS_WH_DOWN), ENCODER_CCW_CW(KC_VOLD, KC_VOLU)  },
    [1] =   { ENCODER_CCW_CW(RGB_HUD, RGB_HUI),           ENCODER_CCW_CW(RGB_SAD, RGB_SAI)  },
    [2] =   { ENCODER_CCW_CW(RGB_VAD, RGB_VAI),           ENCODER_CCW_CW(RGB_SPD, RGB_SPI)  },
    [3] =   { ENCODER_CCW_CW(RGB_RMOD, RGB_MOD),          ENCODER_CCW_CW(KC_RIGHT, KC_LEFT) },
    [4] =   { ENCODER_CCW_CW(RGB_RMOD, RGB_MOD),          ENCODER_CCW_CW(KC_RIGHT, KC_LEFT) },
    [5] =   { ENCODER_CCW_CW(RGB_RMOD, RGB_MOD),          ENCODER_CCW_CW(KC_RIGHT, KC_LEFT) },

};
#endif


//Calling the autoclicker script
bool process_record_user(uint16_t keycode, keyrecord_t *record)
{    if (!process_mouse_turbo_click(keycode, record, TURBO)) {
        return false;
    }

    return true;
}   

//Layer names for OLED
enum layer_names {
    _MAC_DEFAULT,
    _MAC_UTILITY,
    _MAC_MOMENTARY,
    _MAC_MOLAYER,
    _MAC_NUMPAD,
    _MAC_F360

};


#ifdef OLED_ENABLE





// Draw to OLED
bool oled_task_user() {
    //becuase qmk is garbago you have to flash it with the logo array first and then the layer indicators :p -- Should be fixed now 
    
    //Cursor of Oled top left
    oled_set_cursor(0, 0);

    //Macropad name
    oled_write_P(PSTR("EVA-03"), false);
    //Spaces to the left
    oled_write_P(PSTR(" "), false);

        //Display Layer Names
        switch (get_highest_layer(layer_state)) {
        case _MAC_DEFAULT :
   
            oled_write_P(PSTR(" Main   "), false);
                
            break;
        case _MAC_UTILITY :

            oled_write_P(PSTR(" Util   "), false);
       
            break;
        case _MAC_MOMENTARY : 

            oled_write_P(PSTR(" AUX    "), false);
    
            break;
        case _MAC_MOLAYER :       

            oled_write_P(PSTR(" MO     "), false);
       
            break;
        case _MAC_NUMPAD :        

            oled_write_P(PSTR(" Numpad "), false);
      
            break;
        case _MAC_F360 :
 
            oled_write_P(PSTR(" Fusion "), false);
     
            break;
    }
    //Spaces for the image 
    //Byte Array of Image
    static const char PROGMEM raw_logo[] = {
            
                    };
    //Draw logo!                    
    oled_write_raw_P(raw_logo, sizeof(raw_logo));

    return false;
}

#endif